...
public class AlignmentTextViewer extends JPanel {
	...
	public AlignmentTextViewer(final AlignmentConfidences confidences, final int[] scores) {
		...
		if (confidences.hasModels()) {
			this.cmbModels = new JComboBox<>(new Vector<>(confidences.getModels()));
			...
			this.cmbModels.addActionListener(
				new ActionListener() {
					@Override
		...
		final JButton btnDisplayConfiguration = new JButton(
			new AbstractAction("Display Configuration") {
				private static final long serialVersionUID = 1L;
...
//		});
	...
	protected String modelToHtml(boolean inlineStyle) {
		...
		final Map<Integer, Confidence> model = this.confidences.hasModels()
			? this.confidences.getModel((String) this.cmbModels.getSelectedItem())
			: Collections.<Integer, Confidence>emptyMap();

		

		try (BufferedReader reader = new BufferedReader(new InputStreamReader(AlignmentTextViewer.class.getResourceAsStream("alignment.template.html")))) {
					...
					while (offset < seqLength) {
						...
						if (this.configuration.isShowScores()) {
							...
							for (int blockIndex = 0; blockIndex < blocksPerLine; blockIndex++) {
								...
								for (int i = localOffset; i < limit; i++) {
									final char scoreChar = this.scores[i] < 0 ? '-' : Integer.toString(this.scores[i]).charAt(0);
...

		{
			try {
				...
				final JButton btnReset = new JButton(
					new AbstractAction("Reset to Default") {
						private static final long serialVersionUID = 1L;
				...
				cbLiveUpdate.addChangeListener(
					new ChangeListener() {
						@Override
				...
				cbShowScores.addActionListener(
					new ActionListener() {
						public void actionPerformed(ActionEvent e) {
				...
				AlignmentTextViewer.this.configuration.addObserver(
					new Observer() {
