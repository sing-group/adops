...
public class TCoffeeOperation
	...
	@Port(name = "Maximum number of sequences to be removed", order = 3, direction = Direction.INPUT, allowNull = false)
	public void setMaxSeqs(int maxSeqs) {
		this.configuration.setMaxSeqs(maxSeqs);
	}

	@Port(name = "Align method", order = 4, direction = Direction.INPUT, allowNull = false, defaultValue = TCoffeeConfiguration.VALUE_ALIGN_METHOD)
	public void setAlignMethod(AlignMethod method) {
		this.configuration.setAlignMethod(method);
	}

	@Port(name = "Min. Score", order = 4, direction = Direction.INPUT, allowNull = false, defaultValue = TCoffeeConfiguration.VALUE_MIN_SCORE)
	public void setMinScore(int minScore) {
		this.configuration.setMinScore(minScore);
	}

	@Port(name = "Use Std. Output", order = 5, direction = Direction.INPUT, defaultValue = "true")
	...
	public TCoffeeOutput call() throws OperationException, InterruptedException {
		...
		try {
			...
			File finalAlnFile = output.getInitialAlnFile(); // TODO is this the
															// correct file?
			...
			if (maxSeqs > 0) {
				...
				Set<String> remSeqs = new HashSet<>();
