...
public class BatchProject extends Observable implements HasConfiguration, Observer {
	...
	public BatchProject(File folder) throws IllegalStateException {
		...
		if (this.hasAbsoluteFastaPaths()) {
			...
			final List<File> fastaList = new LinkedList<File>();
...
//			this.fastaFiles = this.folder.listFiles(new FilenameFilter() {
//				@Override
//				public boolean accept(File dir, String name) {
//					return name.matches("input[0-9]+.fasta");
//				}
//			});
		}
	...
	public List<Project> listProjects() {
		return new ArrayList<Project>(this.getOutput().getCreatedProjects());
	}
			...
			if (output.isReady(project)) {
				...
				try {
					...
					if (!project.getExperiments().isEmpty()) {
						overridedPropertiesFile = 
							project.getExperiments().get(0).getPropertiesFile();
						FileUtils.copyFile(propertiesFile, overridedPropertiesFile);
					}
				} catch (IOException e) {
					LOG.error(new StringBuilder("Error copying properties file: ")
...
//	private void createProjectsFile() throws IOException {
//		final List<String> projectNames = new ArrayList<String>(this.fastaFiles.length);
//		
//		for (File fastaFile : this.fastaFiles) {
//			projectNames.add(noExtensionFilename(fastaFile.getName()));
//		}
//		
//		FileUtils.writeLines(new File(this.folder, "project.names"), projectNames);
//	}
	
	private void createFastasFile() throws IOException {
		final List<String> fastasNames = new ArrayList<String>(this.fastaFiles.length);
...
//		final File[] dirs = new File[this.fastaFiles.length];
		
//		for (int i = 0; i < this.fastaFiles.length; i++) {
////			dirs[i] = new File(this.folder, "p" + (i+1));
//			dirs[i] = new File(this.folder, noExtensionFilename(this.fastaFiles[i].getName()));
//		}
